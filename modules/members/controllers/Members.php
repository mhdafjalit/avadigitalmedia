...
{
    // Check if user is properly logged in with OTP verification
    if(!$this->session->userdata('user_id') || $this->session->userdata('login_verified') !== TRUE) {
        // Clear any invalid session
        $this->session->sess_destroy();
        delete_cookie('Xcnv');
    ...
    $data['heading_title'] = "Member Dashboard";
	...
	public function index()
{
	...
	public function myaccount(){
		...
		$data['total_music_process_releases'] = count_record('wl_releases'," status IN (5,6) AND album_type='1' ".$condition);
		$data['total_video_process_releases'] = count_record('wl_releases'," status IN (5,6) AND album_type='2' ".$condition);
		$data['total_music_final_releases'] = count_record('wl_releases'," status='1' AND album_type='1' ".$condition);
	...
	{
		redirect('members/release/new_release?album_type=1'); 
		exit;
