		...
		if(is_array($res) && !empty($res))
		{
			...
			if($this->input->post('action')!=''){
				...
				if ($this->form_validation->run() == TRUE)
				{
		            ...
		            $this->session->set_userdata(array('msg_type'=>'success'));
					$this->session->set_flashdata('success', 'Release details have been updated successfully.');
					redirect('members/release/stores/'.$releaseId.$query_str, '');
				}
			}
			$data['res'] = $res;
	...
	public function date_release()
	{
		...
		if(is_array($res) && !empty($res))
		{
			...
			if($this->input->post('action')!=''){
				...
				{
	                $posted_data = array(
	                    'original_release_date_of_music'=>$this->input->post('original_release_date_of_music',TRUE),
		...
		if(is_array($res) && !empty($res))
		{
			...
			if($this->input->post('btn_sbt')!=''){
                ...
                if($is_exist>0){
					...
					redirect('members/album/album_processing'.$query_str, '');
                }
				...
				if ($api_response && isset($api_response['success']) && $api_response['success']) {
                    ...
                    redirect('members/release/submission'.$query_str, '');
                }
                /* API work end*/
			}
			$data['res'] = $res;
