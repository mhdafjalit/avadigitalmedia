<?php 
 //$this->session->set_userdata('playlist_songs_arr', []);
				...
				<div class="mb-3">
					...
					<div class="row g-0">
						...
						<div class="col-2 col-sm-2 mt-1">
			              ...
			              <?php
			              if( $posted_keyword!='') {
			              echo '<a href="'.site_url('members/release/create_playlist').'" class="btn btn-sm btn-outline-danger"><b>Clear</b></a>';
			              }?>
			            </div>
...
$(document).ready(function() {
    ...
    $('.add_song_list,.added').on('click', function(e) {
      ...
      parent_node_obj.addClass('overlay_enable');
        $.ajax({
          url: '<?php echo site_url('members/release/add_playlist_song');?>',
          type: 'POST',
    ...
    playlist_box_obj.on('click','.rm_elements', function(e) {
      ...
      parent_node_obj.addClass('overlay_enable');
        $.ajax({
          url: '<?php echo site_url('members/release/remove_playlist_song');?>',
          type: 'POST',
          data: {'release_id':release_id,btn_sbt:'Y'},
          headers: { XRSP: 'json' },
          dataType: 'json'
