...
if($this->input->post('action') !=''){
	$posted_stores = (!is_array(set_value('release_stores'))) ? []  : array_fill_keys($posted_stores, 1);
}else{
	$posted_stores = $release_stores;	
}
?>
	    	...
	    	<div class="main-content-inner">
				<div class="dash_box p-4">
					<ul class="nav nav-underline tabber_style">
						...
						<li class="nav-item">
							<a class="nav-link active" href="<?php echo site_url('members/release/stores/'.$releaseId.'?album_type='.$album_type);?>">Stores</a>
						</li>
						<li class="nav-item">
							<a class="nav-link" aria-current="page" href="<?php echo site_url('members/release/territories/'.$releaseId.'?album_type='.$album_type);?>">Territories</a>
						</li>
						<li class="nav-item">
							<a class="nav-link" href="<?php echo site_url('members/release/date_release/'.$releaseId.'?album_type='.$album_type);?>">Release Date</a>
						</li>
						<li class="nav-item">
				...
				<?php
		       	echo error_message();
		       	echo form_open(current_url_query_string(),'id="add_release_stores" role="form"');?>
				<div class="dash_box mt-3 overflow-hidden ref_root_ckbox_parent">
					<div class="d-flex justify-content-between border-bottom bg-light p-3">
						<p class="fs-3"><?php echo $heading_title;?></p>
						<p>
					...
					<div class="p-4">
						<div class="row gx-0 gy-2">
							<?php
							foreach ($res_stores as $ckey => $cval) {
							$stores_status = (!empty($posted_stores[$cval['store_id']]) ? 1 : 0);
							echo '<div class="col-sm-6 col-lg-4 form-check">
								  <input class="form-check-input xrec_list" type="checkbox" name="release_stores[]" value="'.$cval['store_id'].'" id="ter'.$cval['store_id'].'" '.(($stores_status=='1')? 'checked': '').'>
								  <label class="form-check-label" for="ter'.$cval['store_id'].'"><img src="'.theme_url().'store_icons/'.$cval['icon'].'" alt="'.$cval['store_title'].'" class="border rounded-2"> '.$cval['store_title'].'</label>
								</div>';
							}?>
						</div>
					</div>
				</div>
				<div class="mt-3 mb-3 text-center"><?php echo form_error('release_stores[]');?></div>
