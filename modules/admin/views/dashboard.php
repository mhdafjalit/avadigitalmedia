	...
	<div class="dash_container">
		...
		<div id="main-content" class="h-100">
			...
			if($this->mres['member_type']=='1'){
				//echo '<div class="dash_company">'.$site_title_text.'</div>';
			}?>
			...
			<div class="main-content-inner">
					...
					foreach ($album_types as $key => $val) {
						echo '<div class="tab-pane fade '.(($key==1)? ' show active':'').'" id="pills-'.$key.'" role="tabpanel" aria-labelledby="pills-'.$key.'-tab" tabindex="0">
							<div id="owl-'.(($key==1)? 'music':'video').'" class="owl-carousel owl-theme mt-2 scroll_arr">
								<div class="item">
									<div class="count_box count_graph1 position-relative overflow-hidden hand trans_eff" onClick="window.open(\''.site_url('admin/album').'?album_type='.$key.'\',\'_parent\')">
								...
								<div class="item">
									<div class="count_box count_graph2 position-relative overflow-hidden hand trans_eff" onClick="window.open(\''.site_url('admin/album/album_processing').'?album_type='.$key.'\',\'_parent\')">
								...
								<div class="item">
									<div class="count_box count_graph3 position-relative overflow-hidden hand trans_eff" onClick="window.open(\''.site_url('admin/album/final_album').'?album_type='.$key.'\',\'_parent\')">
								...
								<div class="item">
									<div class="count_box count_graph1 position-relative overflow-hidden hand trans_eff" onClick="window.open(\''.site_url('admin/album/rejected_album').'?album_type='.$key.'\',\'_parent\')">
								...
								<div class="item">
									<div class="count_box count_graph2 position-relative overflow-hidden hand trans_eff"  onClick="window.open(\''.site_url('admin/album/takedown_album').'?album_type='.$key.'\',\'_parent\')">
					...
					<div class="row g-0">
						<div class="col-lg-9 pe-lg-4">
							<div class="fin_box text-white">
								...
								<p class="float-end">
									<a href="<?php echo site_url('admin/wallet');?>" class="text-white fs-8 fw-bold text-uppercase">View More</a>
								</p>
								<p class="clearfix"></p>
								<div class="row g-0">
									<div class="col-lg-8">
										<div class="row g-3 mt-1">
											<div class="col-sm-4">
												<div class="bg-white text-black text-center rounded-2 pt-2 pb-3">
													<p class="mb-1 fs-4 fw-bold"><img src="<?php echo theme_url();?>images/fin-ico1.svg" alt="" height="35" class="mb-1"><br>
														<?php echo display_price($total_earning);?></p>
											...
											<div class="col-sm-4">
												<div class="bg-white text-black text-center rounded-2 pt-2 pb-3">
													<p class="mb-1 fs-4 fw-bold"><img src="<?php echo theme_url();?>images/money_icon.svg" alt="" height="35" class="mb-1"><br>
														<?php echo display_price($total_dabit);?></p>
													<p class="fs-7 fw-bold">Total Withdrawal</p>
												</div>
											</div>
										</div>
										<p class="mt-4 text-center fin_btn">
											<a href="<?php echo site_url('admin/wallet');?>" title="Request Amount">Request Amount</a> 
									...
									<div class="col-lg-4 financial_sec">
										<div class="graph_w">
											<div id="scheme_stats" class="position-relative" style="height:160px; width:160px; margin:auto;">
												<p class="financial_score">
													<b><?php echo display_price($balance_amt);?></b><br>Available Amount</p>
											</div>
										</div>
						...
						<div class="col-lg-3 mt-4 mt-lg-0">
							<?php
							if(is_array($recent_users) && !empty($recent_users)){?>
							<div class="recently_added text-center text-lg-start">
								<p class="float-start purple fw-bold">Recently Added Users</p>
								<a href="#" title="View More" class="float-end"><img src="<?php echo theme_url();?>images/dot-three.svg" alt=""></a>
								<p class="clearfix"></p>
								<p class="mt-2">Who recently join this platform</p>
								<div class="mt-4 d-flex justify-content-center">
									<?php
									$total_recent_users = count($recent_users) ?? 0;
									foreach ($recent_users as $key => $user) {
									echo '<div class="recently_pic text-center overflow-hidden rounded-circle shadow" data-bs-toggle="tooltip" data-bs-placement="top"data-bs-custom-class="custom-tooltip" data-bs-title="'.$user['first_name'].'"><figure class="align-middle d-table-cell"><img src="'.get_image('profiles',$user['profile_photo'],40,40,'AR').'" alt="'.$user['first_name'].'" width="40" height="40" class="mw-100 mh-100"></figure></div>';
									}
									echo '<p class="mt-3 ms-2"><a href="'.site_url('admin/members').'" class="text-primary fw-bold">+'.$total_recent_users.'</a></p>';
									?>
								</div>
								<p class="text-center mt-4 fs-7 text-uppercase">
									<a href="<?php echo site_url('admin/members');?>" class="text-primary fw-semibold">View More</a>
								</p>
							</div>
							<?php } ?>
						</div>
				...
				<div class="row g-3">
					<div class="col-lg-6">
						<div class="top_paid_box">
							...
							<p><a href="<?php echo site_url('admin/wallet/revenue_graph');?>" class="text-primary fs-8 fw-bold text-uppercase">View More</a></p>
						</div>
					</div>
					<div class="col-lg-6">
						<div class="dash_box p-3 mt-3">
							<p class="float-start heading">Top 10 Paid Tracks</p>
							<a href="<?php echo site_url('admin/release/paid_tracks');?>" title="View More" class="mt-1 float-end text-primary fs-8 fw-bold text-uppercase">View More</a>
							<p class="clearfix"></p>
				...
				<div class="news_area mt-4">
					<p class="float-start heading ms-2">News & Events</p>
					<a href="<?= site_url('events');?>" title="View More" class="blue2 fs-8 mt-2 me-2 fw-bold float-end">VIEW MORE</a>
					<p class="clearfix"></p>
					<div class="dash_sec_bdr mt-3">
						<div class="row g-3">
							...
							$this->load->view('events/events_item',array('val'=>$val));
							echo '</div>';
							}?>
						</div>
					</div>
...
<script>
    ...
    $('#release_month').on('change', function(){
		  ...
		  $.ajax({
		    ...
		    type: 'POST',
		    data: {month: selectedMonth, action: 'Y' },
