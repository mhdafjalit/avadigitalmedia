        ...
        <div class="bg-white p-2 mb-2 rounded-3">
          ...
          <div class="row g-0">
            ...
            <div class="col-3 col-sm-3 mt-1">
              ...
              <?php
              if( $posted_keyword!='') {
              echo '<a href="'.site_url('admin/album/rejected_album?album_type=1').'" class="btn btn-sm btn-outline-danger"><b>Clear</b></a>';
              }?>
            </div>
...
<div class="d-flex align-items-center gap-2 flex-wrap">
    ...
    <a data-fancybox
       ...
       title="View">
        <img src="<?= theme_url();?>images/eye2.svg" width="18" alt="View">
    </a>
    ...
    <!--  Edit -->
    <a href="<?= site_url('admin/release/new_release/'.md5($val['release_id']).'?album_type='.$val['album_type']);?>"
...
<div class="mt-2">
    ...
    <select class="form-select form-select-sm lstatus mb-1" name="status">
        <option value="">Select</option>

        <?php
        if(is_array($album_status_arr)){
            foreach($album_status_arr as $lkey=>$lv){

                if ($lkey != 2 && !($lkey == 1 && $this->mres['member_type'] != 1)) {

                    if (($val['status'] == 3 && $val['is_verify_meta'] == 1) && in_array($lkey, [0,5])) {
                        continue;
                    }
    ...
    <input type="text" name="reason" class="form-control form-control-sm lreason dn mb-1" placeholder="Enter Reason">

    <button class="btn btn-primary btn-sm btn_sbt w-100">Update</button>

    <div class="text-success mt-1" id="msg_<?= $val['release_id'];?>"></div>
...
<?php
echo (($val['status']=='3' || $val['status']=='4') && !empty($val['reason']))
? '<p class="text-secondary"><b>Reason:</b> “ '.$val['reason'].' ”</p>'
: '';
?>
