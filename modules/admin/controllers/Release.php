...
class Release extends Private_Admin_Controller
{
	private $mId;
	...
	// 	$sec_id = ($album_type>1) ? 8 : 7;
	// 	is_access_method($permission_type=2,$sec_id);
	// 	$res = [];
	...
	// 	if($releaseId!=''){
	// 		$res =  get_db_single_row("wl_releases","*"," AND md5(release_id) = '{$releaseId}'");
	// 		if(empty($res))
	// 		{
	...
	// 		$this->form_validation->set_rules('artist_name', 'Artists', 'trim|required|max_length[280]');
	// 		$this->form_validation->set_rules('feature_artist', 'Feature Artists', 'trim|max_length[280]');
	// 		$this->form_validation->set_rules('p_line', 'P Line', 'trim|max_length[180]');
	// 		$this->form_validation->set_rules('version', 'Version/Subtitle','trim|max_length[80]');
	// 		$this->form_validation->set_rules('c_line', 'C Line', 'trim|max_length[180]');
	// 		$this->form_validation->set_rules('production_year', 'Production Year', 'trim|required');
	// 		$this->form_validation->set_rules('upc_ean', 'UPC/EAN', 'trim|regex_match[/^\d{12,13}$/]');
	...
	// 		$this->form_validation->set_rules('go_live_date', 'Physical Release Date', "trim|required");
	// 		$this->form_validation->set_rules('label_name', 'Label Name', "trim|required");
	// 		$this->form_validation->set_rules('producer_catalogue', 'Producer Catalogue Number', "trim|max_length[50]");
	...
	// 				'artist_name'		=> $this->input->post('artist_name',TRUE),
	// 				'feature_artist'	=> $this->input->post('feature_artist',TRUE),
	// 				'upc_ean'			=> $this->input->post('upc_ean',TRUE),
	// 				'go_live_date'		=> $this->input->post('go_live_date',TRUE),
	...
	// 			); 
	// 			 // Only add member_id if user is NOT Super Admin (member_type != 1) 
	// 			 if ($member_type != '1') {
	...
	// 			redirect('admin/release/upload_release/'.md5($inserted_release_id).$query_str, '');
	// 		}
	// 	}
	// 	$data['res'] = $res;
	// 	$data['artists'] = get_db_multiple_row('wl_artists','name,pdl_id,apple_id,spotify_id'," pdl_id!=''");
...
public function new_release()
{
    ...
    if ($releaseId != '') {
        ...
        if (empty($res)) {
            $this->session->set_userdata(array('msg_type' => 'warning'));
            $this->session->set_flashdata('warning', "Record you are trying to update does not exist");
            redirect('admin/release/new_release' . $query_str, '');
        }
    }
    ...
    if ($this->input->post('action') != '') {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            // ============ MEMBER_ID LOGIC (CORRECT) ============
            $releaseId = $this->uri->segment(4);
            
            if (empty($releaseId)) {
                // NEW RELEASE: Always add member_id for all user types
                $posted_data['member_id'] = $member_id;
            } else {
                // EDIT RELEASE: Only add member_id if NOT Super Admin
                if ($member_type != '1') {
                    $posted_data['member_id'] = $member_id;
                }
                // If Super Admin, don't add member_id (keeps existing in DB)
            }
            // ===================================================
            ...
            if (!empty($res) && is_array($res)) {
                ...
                $this->session->set_userdata(array('msg_type' => 'success'));
                $this->session->set_flashdata('success', 'Release details have been updated successfully.');
                $inserted_release_id = $res['release_id'];
            } else {
                $posted_data['created_date'] = $this->config->item('config.date.time');
                $inserted_release_id = $this->admin_model->safe_insert('wl_releases', $posted_data, FALSE);
                $this->session->set_userdata(array('msg_type' => 'success'));
            ...
            $this->db->trans_complete();
            $inserted_release_id = ($inserted_release_id > 0) ? $inserted_release_id : ($res['release_id'] ?? 0);
            redirect('admin/release/upload_release/' . md5($inserted_release_id) . $query_str, '');
        }
    }
		...
		if(is_array($res) && !empty($res))
		{
			...
			if($this->input->post('action')!=''){
				...
				if ($this->form_validation->run() == TRUE)
				{
					...
					redirect('admin/release/tracks/'.$releaseId.$query_str, '');
				}
			}
			$data['res'] = $res;
	...
	public function tracks()
	{
		...
		if(is_array($res) && !empty($res))
		{
			...
			if($this->input->post('action')!=''){
				...
				$this->form_validation->set_rules('song_name','Song Title', 'trim|required|callback_check_song_title_unique['.$release_id.']');
				$this->form_validation->set_rules('composer','Composer','trim|required|max_length[180]');
				$this->form_validation->set_rules('publisher','Publisher','trim|max_length[180]');
				$this->form_validation->set_rules('isrc', 'ISRC','trim|required|regex_match[/^(INAVN\d{7})(,\s?INAVN\d{7})*$/]|callback_unique_release_isrc['.$release_id.']');
				$this->form_validation->set_rules('track_duration', 'Track Duration','trim|regex_match[/^([0-9]{1,2}):([0-5][0-9]):([0-5][0-9])$/]');
				$this->form_validation->set_rules('crbt_title','CRBT Title','trim|max_length[250]');
				$this->form_validation->set_rules('time_for_crbt_cut','Time for CRBT Cut','trim|greater_than[0]');
				$this->form_validation->set_rules('is_isrc', 'Generate an ISRC','trim|required');
				$this->form_validation->set_rules('release_type', 'Release Type','trim|required');
				$this->form_validation->set_rules('content_type', 'Content Type','trim|required');
				$this->form_validation->set_rules('music_director', 'Music Director','trim|required');
				$this->form_validation->set_rules('lyricist', 'Lyricist & Writer','trim|required');
				...
				$this->form_validation->set_rules('track_price', 'Price','trim|numeric|greater_than_equal_to[0]');
				$this->form_validation->set_rules('is_various_artist', 'Various Artist', "trim");
				if ($this->form_validation->run() == TRUE)
				{
					$time_for_crbt_cut = (int) $this->input->post('time_for_crbt_cut',TRUE);
					$posted_data = array( 
						"prim_track_type"	=> $this->input->post('prim_track_type',TRUE),
						'release_type'		=> $this->input->post('release_type',TRUE),
						'content_type'		=> $this->input->post('content_type',TRUE),
						'is_instrumental'	=> $this->input->post('is_instrumental',TRUE),
						'crbt_title'		=> $this->input->post('crbt_title',TRUE),
						'time_for_crbt_cut'	=> $time_for_crbt_cut,
						'song_name'			=> $this->input->post('song_name',TRUE),
						'composer'			=> $this->input->post('composer',TRUE),
						'music_director'	=> $this->input->post('music_director',TRUE),
						'publisher'			=> $this->input->post('publisher',TRUE),
						'isrc'				=> $this->input->post('isrc',TRUE),
						'track_duration'	=> $this->input->post('track_duration',TRUE),
						'lyricist'			=> $this->input->post('lyricist',TRUE),
						'is_isrc'			=> $this->input->post('is_isrc',TRUE),
						'song_mood'			=> $this->input->post('song_mood',TRUE),
		            ...
		            $this->session->set_userdata(array('msg_type'=>'success'));
					$this->session->set_flashdata('success', 'Release details have been updated successfully.');
					redirect('admin/release/stores/'.$releaseId.$query_str, '');
				}
			}
			$data['res'] = $res;
		...
		if(is_array($res) && !empty($res))
		{
			...
			if($this->input->post('action')!=''){
				...
				if ($this->form_validation->run() == TRUE)
				{
					...
					redirect('admin/release/territories/'.$releaseId.$query_str, '');
				}
			}
	...
	public function territories()
	{
		...
		if(is_array($res) && !empty($res))
		{
			...
			if($this->input->post('action')!=''){
				$this->form_validation->set_rules('release_territories[]', 'Select Relevant Territories','trim');
				if ($this->form_validation->run() == TRUE)
				{
					...
					redirect('admin/release/date_release/'.$releaseId.$query_str, '');
				}
			}
			$params_terr = array(
					'fields'=>"wtc.*,wt.name",
	...
	public function date_release()
	{
		...
		if(is_array($res) && !empty($res))
		{
			...
			$this->mem_top_menu_section = 'release';
			if($this->input->post('action')!=''){
				$this->form_validation->set_rules('original_release_date_of_music','Original Music Release Date',"trim|required");
				// $this->form_validation->set_rules('main_release_state', 'Main Release State', "trim");
				if ($this->form_validation->run() == TRUE)
				{
	                $posted_data = array(
	                    'original_release_date_of_music'	=>$this->input->post('original_release_date_of_music'),
						//'main_release_state'=>$this->input->post('main_release_state'),
	                    'status'			=> '0',
	                    'updated_date'  	=> $this->config->item('config.date.time')
	                );
					$posted_data = $this->security->xss_clean($posted_data); 
					...
					redirect('admin/release/submission/'.$releaseId.$query_str, '');
				}
			}
			$data['res'] = $res;
	...
	public function submission()
	{
		...
		if(is_array($res) && !empty($res))
		{
			...
			if($this->input->post('btn_sbt')!=''){
                ...
                $this->admin_model->safe_update('wl_releases',$posted_data,['release_id'=>$release_id], FALSE);
                /* API work */
                $is_instrumental = $res['is_instrumental']>0 ? 'Yes' : 'No';
                $artist_id = $res['artist_name'];
                $artist_name = get_db_field_value('wl_artists','name',['pdl_id'=>$artist_id]);
                $time_for_crbt_cut = gmdate("H:i:s", $res['time_for_crbt_cut']?? 0);
                $audio_file = $res['release_song'] ?? '';
				$image_file = $res['release_banner'] ?? '';
				$image_file_path = ($image_file != '' && file_exists(UPLOAD_DIR . '/release/' . $image_file)) ? UPLOAD_DIR . '/release/' . $image_file : null;
				$audio_file_path = ($audio_file != '' && file_exists(UPLOAD_DIR . '/release/songs/' . $audio_file)) ? UPLOAD_DIR . '/release/songs/' . $audio_file : null;
				$image_file_size_kb = ($image_file_path && file_exists($image_file_path)) ? filesize($image_file_path) : null;
				$audio_file_size_kb = ($audio_file_path && file_exists($audio_file_path)) ? filesize($audio_file_path) : null;
				$image_md5 = ($image_file_path && file_exists($image_file_path)) ? md5_file($image_file_path) : null;
				$audio_md5 = ($audio_file_path && file_exists($audio_file_path)) ? md5_file($audio_file_path) : null;
				$is_exist = count_record('wl_signed_albums',"release_ref_id='{$release_id}' ");
                if($is_exist>0){
                	$this->session->set_userdata(array('msg_type'=>'success'));
					$this->session->set_flashdata('success', 'The album has finally been submitted and is now in processing.');
                ...
                $api_payload = [
					...
					"albums" => [
						[
							"is_update" => false,
							"name" => $res['release_title'],
							"label" => $res['label_name'],
							"c_line" => $res['c_line'] ?? '',
							"upc_id" => $res['upc_ean'] ?? '',
							"p_line" => $res['p_line'] ?? '',
							"film_banner" => $res['film_banner'] ?? '',
							...
							"songs" => [
								[
									...
									"data" => [
										"crbt_cut_name" => $res['crbt_title'] ?? $res['song_name'],
										"song_name" 	=> $res['song_name'],
										"album_name" 	=> $res['release_title'],
										"language" 		=> $lang_arr[$res['lyrics_lang']],
										"album_type" 	=> $res['release_type'],
										...
										"label" 		=> $res['label_name'],
										"publisher" 	=> $res['publisher'] ?? '',
										"track_duration" => $res['track_duration'] ?? '0:00:00',
										"crbt_start_time" => $time_for_crbt_cut ?? '0:00:00',
										"time_for_crbt_cut" => $time_for_crbt_cut ?? '0:00:00',
										"original_release_date_of_movie" => $res['go_live_date'] ?? '',
										"original_release_date_of_music" => $res['go_live_date'] ?? '',
										"go_live_date" 		=> $res['original_release_date_of_music'] ?? null,
										"date_of_expiry" 	=> $res['date_of_expiry'] ?? null,
										"c_line" 			=>$res['c_line'] ?? '',
										"p_line" 			=> $res['p_line'] ?? '',
										"film_banner"		=> $res['film_banner'] ?? '',
										"parental_advisory (EXPLICIT ETC)" => $res['parental_advisory'] ?? '',
							...
							"album_main_artist" => [],
				            "album_featured_artist" => [],
				            "album_remixer_artist" => [],
				            "film_director" => [],
				            "film_producer" => [],
				            "film_starcast" => []
						]
				...
				if ($api_response && isset($api_response['success']) && $api_response['success']) {
					$inserted_id = $this->insertAlbum($api_response,$release_id);
					...
					$image_orignal_path = file_get_contents($image_file_path);
					$media_orignal_path = file_get_contents($audio_file_path);
					$img_ext = pathinfo($image_orignal_path, PATHINFO_EXTENSION);
					$media_ext = pathinfo($media_orignal_path, PATHINFO_EXTENSION);
					$response1 = $this->api_service->makeAlbumRequest('PUT', $image_endpoint, $image_orignal_path, $img_ext);
					$response2 = $this->api_service->makeAlbumRequest('PUT', $media_endpoint, $media_orignal_path, $media_ext);
                    ...
                    redirect('admin/release/submission'.$query_str, '');
                }
                /* API work end*/
			}
			$data['res'] = $res;
	...
	public function insertAlbum($api_response, $release_ref_id)
	{
	    try {
	        ...
	        $song_data = $song['data'] ?? [];
	        $insert_data = [
	            'release_ref_id' => $release_ref_id ?? 0,
	            ...
	            'mood'        => $song_data['mood'] ?? '',
	            'lyric'     => $song['lyricists'][0]['name'] ?? '',
	            'composser' => $song['composers'][0]['name'] ?? '',
	            'music_director' => $song['directors'][0]['name'] ?? '',
	            'media_type' => pathinfo($song['media']['filename'] ?? '', PATHINFO_EXTENSION),
	            'go_live_date' => !empty($song_data['go_live_date'])
	                ? date('Y-m-d', strtotime($song_data['go_live_date']))
	                : null,
	            'date_of_expiry' => !empty($song_data['date_of_expiry'])
	                ? date('Y-m-d', strtotime($song_data['date_of_expiry']))
	                : null,
	            'warnings' => !empty($album['warnings'])
	                ? json_encode($album['warnings'])
	                : null,
	...
	public function view_meta_release()
	{
		$album_type = (int) $this->input->get_post('album_type');
		$sec_id = ($album_type>1) ? 8 : 7;
		is_access_method($permission_type=1,$sec_id);
		...
		if(is_array($res) && !empty($res))
		{
			...
			$this->mem_top_menu_section = 'release';
			$data['res'] = $res;
			$this->load->view('release/view_final_meta_release',$data);
		}
		else
		{
			$this->session->set_userdata(array('msg_type'=>'warning'));
	...
	public function create_playlist(){
		...
		if($this->input->post('action')!=''){
			...
			if(empty($playlist_songs_arr)){
				...
				redirect('admin/release/create_playlist'.$query_str);
			}
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				redirect('admin/album/playlists'.$query_str, '');
			}
		}
	...
	public function add_playlist_song() {
	    if ($this->input->post('btn_sbt') != '') {
	        ...
	        $this->form_validation->set_rules('release_id', 'Id', "trim|required");
	...
	public function check_song_title_unique($song_title, $release_id = null)
	{
	    if ($this->session->userdata('member_type') == 1) return TRUE;
	    $this->db->where('song_name', $song_title)->where('status !=', 2);
	    if ($release_id) {
	        $this->db->where('release_id !=', $release_id);
	    }
	    if ($this->db->count_all_results('wl_releases')) {
	        $this->form_validation->set_message('check_song_title_unique','This {field} already exists');
	        return FALSE;
	    }
	    return TRUE;
	}

	public function unique_release_isrc($str,$release_id = null)
	{
	    $this->db->where('isrc', $str);
	    $this->db->where('status !=', '2');
	    if ($release_id) {
	        $this->db->where('release_id !=', $release_id);
	    }
	    $query = $this->db->get('wl_releases');
	    if ($query->num_rows() > 0) {
	        $this->form_validation->set_message('unique_release_isrc', 'The {field} field must be unique.');
	        return FALSE;
	    }
	    return TRUE;
	}
	...
	public function download_labels() {
	    $qry = $this->db->query("SELECT label_id, channel_name FROM wl_labels");
	    if ($qry->num_rows() > 0) {
	        $res = $qry->result_array();
	        $spreadsheet = new Spreadsheet();
	...
	public function download_release_format() {
	    ...
	    $qry = $this->db->query("SELECT " . $column_fields . " FROM wl_releases WHERE status != '2' AND album_type = ? LIMIT 12", [$album_type]);
	    if ($qry->num_rows() > 0) {
	        $res = $qry->result_array();
		...
		if($this->input->post('action')!=''){
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if (is_array($release_bulk_data) && !empty($release_bulk_data)) {
					...
					redirect('admin/release/bulk_release', '');
				}
			}
		}
