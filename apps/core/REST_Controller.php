...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if(class_exists('Api_rest_format'))
        {
            $this->format = new Api_rest_format();
        }
        else
        {
            $this->load->library('api_rest_format', NULL, 'libraryFormat');
        ...
        $this->rest = new stdClass();
				$this->default_api_key = $this->config->item('default_api_key');
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        {
						$response = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unsupported'),'error'=>$this->lang->line('text_rest_unsupported'),'err_code'=>'AUTH_RMP401');
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            ...
            if($this->request->method == 'options') {
                exit;
						...
						$msg = sprintf($this->lang->line('text_rest_invalid_api_key'), $this->rest->key);
						$response = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $msg,'error'=>$msg,'err_code'=>'AUTH_RMP402');
            $this->response($response, self::HTTP_FORBIDDEN);
            $this->is_valid_request = false;
        }
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === FALSE && $this->_check_access() === FALSE)
        {
            if ($this->config->item('rest_enable_logging') && $log_method)
            {
                $this->_log_request();
						...
						$response = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_unauthorized'),'error'=>$this->lang->line('text_rest_api_key_unauthorized'),'err_code'=>'AUTH_RMP403');
        ...
        if (! method_exists($this, $controller_method))
        {
						$response = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unknown_method'),'error'=>$this->lang->line('text_rest_unknown_method'),'err_code'=>'AUTH_RMP404');
            $this->response($response, self::HTTP_METHOD_NOT_ALLOWED);
            $this->is_valid_request = false;
        }
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE)
        {
            ...
            {
								$response = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_time_limit'),'error'=>$this->lang->line('text_rest_api_key_time_limit'),'err_code'=>'AUTH_RMP405');
                $this->response($response, self::HTTP_UNAUTHORIZED);
                $this->is_valid_request = false;
            }
            ...
            if($authorized === FALSE)
            {
                // They don't have good enough perms
								$response = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_permissions'),'error'=>$this->lang->line('text_rest_api_key_permissions'),'err_code'=>'AUTH_RMP406');
                $this->response($response, self::HTTP_UNAUTHORIZED);
                $this->is_valid_request = false;
            }
        }
        ...
        { 
						$response = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_address_time_limit'),'error'=>$this->lang->line('text_rest_ip_address_time_limit'),'err_code'=>'AUTH_RMP407');
            $this->response($response, self::HTTP_UNAUTHORIZED);
            $this->is_valid_request = false;
        }
    ...
    protected function _detect_api_key()
    {
        ...
        {
						$this->request_token = $key;
            if ( ! ($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->where('expired_at IS NULL')->get($this->config->item('rest_keys_table'))->row()))
            {
								if($key==$this->default_api_key){
									 $this->rest->key = $key;
									 return TRUE;
								}
                return FALSE;
    ...
    protected function _check_php_session()
    {
        ...
        {
            // Display an error response
						$data = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unauthorized'),'error'=>$this->lang->line('text_rest_unauthorized'),'err_code'=>'PSA401');
    ...
    protected function _prepare_digest_auth()
    {
        ...
        {
            // Display an error response
						$data = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_invalid_credentials'),'error'=>$this->lang->line('text_rest_invalid_credentials'),'err_code'=>'DGA401');
            $this->response($data, self::HTTP_UNAUTHORIZED);
        }
    }
    /**
    ...
    protected function _check_blacklist_auth()
    {
        ...
        {
            // Display an error response
						$data = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_denied'),'error'=>$this->lang->line('text_rest_ip_denied'),'err_code'=>'BLA401');
            $this->response($data, self::HTTP_UNAUTHORIZED);
        }
    }
    /**
    ...
    protected function _check_whitelist_auth()
    {
        ...
        {
						$data = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_unauthorized'),'error'=>$this->lang->line('text_rest_ip_unauthorized'),'err_code'=>'WLA401');
            $this->response($data, self::HTTP_UNAUTHORIZED);
        }
    }
    /**
    ...
    protected function _force_login($nonce = '')
    {
        ...
        // Display an error response
				$data = array('success'=>false, $this->config->item('rest_status_field_name') => FALSE,$this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unauthorized'),'error'=>$this->lang->line('text_rest_unauthorized'),'err_code'=>'FL101');
        $this->response($data, self::HTTP_UNAUTHORIZED);
    }
    /**
    ...
    protected function _check_cors()
    {
		...
		/**
     * Perform Token Library Authentication - Override this function to change the way the library is called
     *
     * @access protected
     * @param string $token The token to validate
     * @return bool
     */
    protected function _perform_library_token_auth($token)
    {
        if (empty($token))
        {
            log_message('error', 'Library Token Auth: Failure, empty token');
            return FALSE;
        }
        ...
        if (!class_exists($auth_library_class,false))
        {
            $this->load->library($auth_library_class);
						
        }
        return $this->{$auth_library_class}->$auth_library_function($token);
    }

		 /**
     * Prepares for token authentication
     *
     * @access protected
     * @return void
     */
    protected function _prepare_token_auth()
    {
        ...
        $http_auth = $this->input->server('HTTP_AUTHENTICATION');
				$auth_source = strtolower($this->config->item('auth_source'));
				if($auth_source=='library'){
					log_message('debug', "Performing token Library authentication for $http_auth");

					$result = $this->_perform_library_token_auth($http_auth);
					if(!$result){
						$data = array('success'=>false,'error'=>'Credential not valid.');
						$this->response($data, self::HTTP_UNAUTHORIZED);
					}
				}
