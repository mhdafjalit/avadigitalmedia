...
require_once(APPPATH.'libraries/import/utility/inventory/Inventory_item_import.php');
require_once(APPPATH.'libraries/import/csv/inventory/Inventory_base_import_csv.php');
use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Csv;
#[AllowDynamicProperties]
class Inventory_item_import_csv extends Inventory_base_import_csv {
	use Inventory_item_import;
	...
	public function __construct($params=array()){
		...
		$this->ci->load->helper(array('inventory/inventory','seo/seo'));
		...
		$this->config_inventory_fields_maxlength = $this->ci->config->item('CX_INVENTORY_FIELDS_MAXLENGTH');
		$this->max_custom_fields = $this->ci->config->item('cx_inv_max_custom_fields');
		$this->valid_item_classes = array("Stock Item","Master Stock Item","Substock Item","Non Stock Item","Assembly","Activity Item","Charge Item","Service","Labor","Description Only");
	}

	public function create_sample_for_import($params=array()){
		ini_set('memory_limit','-1');
		$filename=$params['filename'] ?? 'sample_inventory_item_'.date("Y-m-d H:i:s");
	...
	protected function validate_data(){
		...
		*************/
		...
		if($this->debug_mode===TRUE){
			...
			{
				echo "<p>Data after the item sheet traversed</p>";
