...
require_once(APPPATH.'libraries/import/utility/vendor/Vendor_import.php');
require_once(APPPATH.'libraries/import/csv/vendor/Vendor_base_import_csv.php');
use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Csv;
#[AllowDynamicProperties]
class Vendor_import_csv extends Vendor_base_import_csv {
	use Vendor_import;
	...
	public function create_sample_for_import($params=array()){
		...
		$filename=$params['filename'] ?? 'sample_vendor_'.date("Y-m-d H:i:s");
		...
		$spreadsheet->getSheet(0)->setTitle("Vendors");
	...
	protected function validate_data(){
		$vendor_worksheet  = $this->sheet_refs[0];
		//Debug Control
		...
		*************/
		...
		$highest_entry = $vendor_worksheet->getHighestRow();

		$total_vendor_entries = !empty($this->is_preview) ? 5 : (!isset($this->data_row_end) ? $highest_entry  : $this->data_row_end);

		if($total_vendor_entries>$highest_entry){
			$total_vendor_entries = $highest_entry;
		}
		...
		$error_rows = array();
		$firstRow = $vendor_worksheet->getRowIterator()->current();
		$headings = [];
		...
		for($ox=$this->data_row_start;$ox<=$total_vendor_entries;$ox++){
			$loop_row = $vendor_worksheet->getRowIterator()->seek($ox)->current();
			$is_row_empty = self::isRowEmpty($loop_row,array());
			if($is_row_empty===TRUE){
				continue;
			}
			$vendor_data_index = array_push($vendor_data_row, array('row_index'=>$ox,'error'=>array(),'data'=>array()));
			$vendor_data_index = $vendor_data_index-1;
			$loop_error_obj = &$vendor_data_row[$vendor_data_index]['error'];
			$loop_data_obj = &$vendor_data_row[$vendor_data_index]['data'];
			$loop_has_data_error=FALSE;
			$this->loop_remit_address_data = array();
			$this->loop_data_ps_info_setting = array();
			...
			$loop_data_obj['remit_addresses'] = $this->loop_remit_address_data;
			$loop_data_obj['ps_info_setting'] = $this->loop_data_ps_info_setting;
			$vendor_data_row[$vendor_data_index]['data'] = $loop_data_obj;
			if(!$this->apply_validation){
				$this->data_results = array($vendor_data_row[$vendor_data_index]);
				$this->upload_data();
			}
			if(!empty($loop_error_obj)){
				$error_rows['zx_'.$ox] = array('row'=>$ox,'error'=>$loop_error_obj);
			}
			//$vendor_data_row[$ox]['error'] = $loop_error_obj;
		}

		if($this->debug_mode===TRUE){
			if($debug_level==1 || $debug_level==-1)
			{
				echo "<p>Data after the vendor sheet traversed</p>";
				echo '<p>Manipulated Data Tree</p>';
				trace($vendor_data_row);
		...
		if(!$this->has_data_error){
			$total_existing_records = count($vendor_data_row);
			$this->has_data_error = !$total_existing_records;
		}
		$ret_vendor_data_row = array(
															'has_data_error'=>$this->has_data_error,
															'data_row_result'=>$vendor_data_row,
															'error_rows'=>&$error_rows
														);
		$vendor_data_row = null;
		return $ret_vendor_data_row;
	}
