    ...
    public function to_json($data = NULL)
    {
        ...
        if ($data === NULL && func_num_args() === 0)
        {
            $data = $this->_data;
				...
				if(is_array($data) && function_exists('my_string_array')){
					$data = my_string_array($data);
					//Hack to append fixed data in all the apis
					if(!isset($data['root_category_id'])){
						$root_category_id=(int) (isset($_POST['root_category_id']) ? $_POST['root_category_id'] : 0);
