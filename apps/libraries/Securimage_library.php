...
class Securimage_library {
  ...
  public function initialize($params=array())
  {
	...
	if ($mobile == true)
	{
		switch($mobile)
		{
			...
			break;
			case 'custom_user':
			  include(APPPATH.'config/securimage_user'.EXT);
			break;
			default:
			  include(APPPATH.'config/securimage_mobile'.EXT);
		}
 ...
 public function saveData()
  {
	$session_captcha_data = @$this->CI->session->userdata('securimage_code_value');
	if(!is_array($session_captcha_data))
	{
	  $session_captcha_data = array();
	}
	if(!array_key_exists($this->namespace,$session_captcha_data))
	{
	  $session_captcha_data[$this->namespace] = '';
	}
	$session_captcha_data[$this->namespace] = $this->code;

	$this->CI->session->set_userdata('securimage_code_value', $session_captcha_data);
  }
  ...
  public function validate()
  {
	$session_captcha_data = $this->CI->session->userdata('securimage_code_value');
//echo "===".$this->namespace;
    $securimage_code_value = @$session_captcha_data[$this->namespace];
	if ( isset($securimage_code_value) && !empty($securimage_code_value) ) {
      if ($securimage_code_value == trim($this->code_entered) ) {
		  $this->correct_code = true;
		  unset($session_captcha_data[$this->namespace]);
