...
if ( ! function_exists('get_prim_artists_old'))
{
    function get_prim_artists_old($release_id)
    {
        $CI = CI();
...
if ( ! function_exists('get_prim_artists'))
{
    ...
    {
        $CI = CI();
        $prim_artists = $CI->db->select('name')->from('wl_artists')->where('pdl_id',$release_id)->get()->result_array();
        return implode(", ", array_column($prim_artists, 'name'));
    }
}
