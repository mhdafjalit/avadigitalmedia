...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        {
            $this->request->body = $this->rest_format->factory($this->request->body, $this->request->format)->to_array();
    ...
    public function response($data = NULL, $http_code = NULL)
    {
        ...
        elseif ($data !== NULL)
        {
            ...
            if (method_exists($this->rest_format, 'to_' . $this->response->format))
            {
                // Set the format header
                $this->output->set_content_type($this->_supported_formats[$this->response->format], strtolower($this->config->item('charset')));
                $output = $this->rest_format->factory($data)->{'to_' . $this->response->format}();

                // An array must be parsed as a string, so as not to cause an array to string error
                // Json is the most appropriate form for such a data type
                if ($this->response->format === 'array')
                {
                    $output = $this->rest_format->factory($output)->{'to_json'}();
                }
            }
            else
            {
                ...
                {
                    $data = $this->rest_format->factory($data)->{'to_json'}();
                }
