...
class Album extends Private_Controller
{
	...
	public function album_created()
	{
		...
		$data['heading_title'] = "All Releases";
	...
	public function album_processing(){
		...
		$album_type 	= (int) $this->input->get_post('album_type');
		$keyword 		= $this->db->escape_str( $this->input->get_post('keyword',TRUE));
		$condition 	    ="wr.status = '0' AND wr.member_id='".$this->userId."'";
		if($album_type>0){
			$condition .=" AND wr.album_type = '".$album_type."'";
		}
		if($keyword!='')
	...
	public function final_album(){
		...
		$album_type 	= (int) $this->input->get_post('album_type');
		$keyword 		= $this->db->escape_str( $this->input->get_post('keyword',TRUE));
		$condition 	    ="wr.status = '1' AND wr.member_id='".$this->userId."'";
		if($album_type>0){
			$condition .=" AND wr.album_type = '".$album_type."'";
		}
		if($keyword!='')
	...
	public function rejected_album(){
		...
		$album_type 	= (int) $this->input->get_post('album_type');
		$keyword 		= $this->db->escape_str( $this->input->get_post('keyword',TRUE));
		$condition 	    ="wr.status = '3' AND wr.member_id='".$this->userId."'";
		if($album_type>0){
			$condition .=" AND wr.album_type = '".$album_type."'";
		}
		if($keyword!='')
	...
	public function takedown_album(){
		...
		$album_type 	= (int) $this->input->get_post('album_type');
		$keyword 		= $this->db->escape_str( $this->input->get_post('keyword',TRUE));
		$condition 	    ="wr.status = '4' AND wr.member_id='".$this->userId."'";
		if($album_type>0){
			$condition .=" AND wr.album_type = '".$album_type."'";
		}
		if($keyword!='')
	...
	public function album_status()
	{
		...
		if($is_exist>0)
		{
			...
			redirect('members/album/'.$red_url); 
		}
		else
		{
			$this->session->set_userdata(array('msg_type'=>'error'));
			$this->session->set_flashdata('error',"Something Went Wrong.");
			redirect('members/album'); 
		}
	}
	...
	public function release_delete()
	{
		...
		if($is_exist>0)
		{
			...
			redirect('members/album/album_created');
		}
		else
		{
			$this->session->set_userdata(array('msg_type'=>'error'));
	...
	public function playlist_delete()
	{
		...
		if($is_exist>0)
		{
			...
			redirect('members/album/playlists');
		}
		else
		{
			$this->session->set_userdata(array('msg_type'=>'error'));
