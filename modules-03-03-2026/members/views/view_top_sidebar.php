...
$where_notification="nc.customer_id='".$this->userId."' AND wn.status='1' AND nc.read_status='U'";
		...
		<div class="dashboard_right float-end d-flex justify-content-between">
			...
			<div>
				...
				<div class="dropdown alert_top d-inline-block align-top position-relative">
					<button type="button" class="btn border-0 p-0 rounded-circle d-block text-center trans_eff" data-bs-toggle="dropdown" aria-expanded="false"><img src="<?php echo theme_url();?>images/bell.svg" alt=""><?php if(is_array($res_notifications) && !empty($res_notifications)){?><img src="<?php echo theme_url();?>images/dot.svg" alt="" class="alert_top_dot position-absolute rounded-circle"><?php } ?></button>
					<div class="dropdown-menu alert_box">
						...
						if(is_array($res_notifications) && !empty($res_notifications)){
							foreach($res_notifications as $key=>$val){
								echo '<a href="'.site_url('members/notifications').'">'.$val['notification_title'].'</a>';
							}
                ...
                <a href="<?php echo site_url('members/release');?>" title="Create Releases" class="create_top text-white rounded-5 fw-medium trans_eff align-middle"><img src="<?php echo theme_url();?>images/create.svg" width="26" alt="" class="me-0 me-md-1"> <span class=" d-none d-sm-inline-block ">Create Releases</span></a>
                
				<p class="setting_top d-inline-block align-top">
					<a href="<?php echo site_url('members/edit_account');?>" title="Setting" class="rounded-circle d-block text-center trans_eff">
