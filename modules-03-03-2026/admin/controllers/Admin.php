...
class Admin extends Private_Admin_Controller
{
	...
	public function myaccount(){
        ...
        */
        $conditions = ["wm.status != '2'"];
        if ($member_type == '2' && $member_id > 0) {
            $conditions[] = "(cus.parent_id = '{$member_id}' OR wm.member_id = '{$member_id}')";
        }
        
        $where = function($extra) use ($conditions) {
            return implode(" AND ", $conditions) . ($extra ? " AND $extra" : "");
        };
        
        $data['total_music_releases'] = count_record_with_join(
            'wl_signed_albums as wm',
            "LEFT JOIN wl_customers as cus ON cus.customers_id = wm.member_id AND cus.status='1'",
            $where("wm.is_verify_meta='0' AND wm.is_pdl_submit='0'")
        );
        $data['total_music_process_releases'] = count_record_with_join(
            'wl_signed_albums as wm',
            "LEFT JOIN wl_customers as cus ON cus.customers_id = wm.member_id AND cus.status='1'",
            $where("wm.is_verify_meta='1' AND wm.is_pdl_submit='0'")
        );
        $data['total_music_final_releases'] = count_record_with_join(
            'wl_signed_albums as wm',
            "LEFT JOIN wl_customers as cus ON cus.customers_id = wm.member_id AND cus.status='1'",
            $where("wm.is_verify_meta='1' AND wm.is_pdl_submit='1'")
        );
		
		
		//$data['total_music_releases'] = count_record('wl_releases'," status='0' AND album_type='1'");
		$data['total_video_releases'] = count_record('wl_releases'," status='0' AND album_type='2'");
	...
	public function export_signed_albums($metadata_from, $metadata_to) {
		...
		$this->db->from('wl_signed_albums');
		$this->db->where('created_date >=', $metadata_from);
		$this->db->where('created_date <=', $metadata_to);
