...
class Metas extends Private_Controller {
    ...
    public function __construct() {
        ...
        $this->meta_fields = [
            'album' => [
                ...
                'c_line' => ['required' => false, 'label' => 'C Line'],
                'upc_id' => ['required' => false, 'label' => 'UPC'],
                'p_line' => ['required' => false, 'label' => 'P Line'],
            ...
            'song' => [
                ...
                'crbt_cut_name' => ['required' => false, 'label' => 'CRBT Cut Name'],
                'song_name' => ['required' => true, 'label' => 'Song Name'],
                ...
                'original_release_date_of_movie' => ['required' => false, 'label' => 'Original Movie Release Date'],
                'original_release_date_of_music' => ['required' => true, 'label' => 'Original Music Release Date'],
                'go_live_date' => ['required' => false, 'label' => 'Go Live Date'],
                'date_of_expiry' => ['required' => false, 'label' => 'Date of Expiry'],
    ...
    public function index() {        
		...
		$data['heading_title'] = "Manage PDL Meta";
		...
		if($keyword!='') {
			$condition .= " AND (wm.song_name like '%$keyword%') ";
		}
		if($artist != '') {
	...
	public function toggle_is_new_released($id) {
		$this->toggle_meta_flag($id, 'is_new_released');
	}
	
	public function toggle_is_recently_added($id) {
		$this->toggle_meta_flag($id, 'is_recently_added');
	}
	
	public function toggle_is_latest($id) {
		$this->toggle_meta_flag($id, 'is_latest');
	}
	
	public function toggle_is_top_rated($id) {
		$this->toggle_meta_flag($id, 'is_top_rated');
	}
	
	private function toggle_meta_flag($id, $field) {
		...
		$this->db->where('id', $id);
		$result = $this->db->update('wl_signed_albums', [$field => $new_value]);
    ...
    public function add() {
        ...
        if ($this->input->post()) {					
            ...
            $this->form_validation->set_rules('song[song_name]', 'Song Name', 'required');
            ...
            if ($this->form_validation->run() !== FALSE) {
							...
							"songs" => [
								[
									...
									"data" => [
										...
										"time_for_crbt_cut" => $post_data['song']['time_for_crbt_cut'] ?? '0:00:30',
										"original_release_date_of_movie" => $post_data['song']['original_release_date_of_movie'] ?? '',
										...
										"parental_advisory (EXPLICIT ETC)" => $post_data['song']['parental_advisory'],
				...
				if ($api_response && isset($api_response['success']) && $api_response['success']) {
					...
					$response1 = $this->api_service->makeAlbumRequest('PUT', $image_endpoint, $image_orignal_path); // success return 200
					
					$response2 = $this->api_service->makeAlbumRequest('PUT', $media_endpoint, $media_orignal_path); // success return 200
		...
		try {
			...
			if ($response['success']) {
				...
				$this->db->where('id', $mata_id)
						 ->update('wl_signed_albums', ['is_verify_meta' => '1']);
			} else {
